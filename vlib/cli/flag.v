...
pub fn (flag &Flag) get_bool() ?bool {
	if flag.kind != .bool {
		return error('$flag.name: Invalid flag type `$flag.kind`, expected `bool`')
	}
...
pub fn (flag &Flag) get_int() ?int {
	if flag.kind != .int {
		return error('$flag.name: Invalid flag type `$flag.kind`, expected `int`')
	}
...
pub fn (flag &Flag) get_ints() ?[]int {
	if flag.kind != .int_array {
		return error('$flag.name: Invalid flag type `$flag.kind`, expected `int_array`')
	}
...
pub fn (flag &Flag) get_float() ?f64 {
	if flag.kind != .float {
		return error('$flag.name: Invalid flag type `$flag.kind`, expected `float`')
	}
...
pub fn (flag &Flag) get_floats() ?[]f64 {
	if flag.kind != .float_array {
		return error('$flag.name: Invalid flag type `$flag.kind`, expected `float_array`')
	}
...
pub fn (flag &Flag) get_string() ?string {
	if flag.kind != .string {
		return error('$flag.name: Invalid flag type `$flag.kind`, expected `string`')
	}
...
pub fn (flag &Flag) get_strings() ?[]string {
	if flag.kind != .string_array {
		return error('$flag.name: Invalid flag type `$flag.kind`, expected `string_array`')
	}
...
fn (mut flag Flag) parse(args []string, with_abbrev bool) ?[]string {
	if flag.matches(args, with_abbrev) {
		if flag.kind == .bool {
			...
			if flag.value.len > 0 && flag.kind != .int_array && flag.kind != .float_array
				&& flag.kind != .string_array {
				return error('The argument `$flag.name` accepts only one value!')
			}
...
fn (flag &Flag) get_value_or_default_value() []string {
	if flag.value.len == 0 && flag.default.len > 0 {
